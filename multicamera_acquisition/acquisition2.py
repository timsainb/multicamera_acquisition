...
def acquire_video(
    ...
    for camera_dict in camera_list:
        ...
        if verbose:
            logging.info(f"Creating acquisition loop {name} ({serial_number})")
        ...
        if verbose:
            logging.info(f"Starting acquisition loop {name} ({serial_number})")
    ...
    if verbose:
        logging.info(f"Completed camera initialization")

    if verbose:
            logging.info(f"Creating multi camera display")
    ...
    for acquisition_loop in acquisition_loops:
        ...
        if acquisition_loop.is_alive():
            logging.log("Failed to stop acquisition loop")
